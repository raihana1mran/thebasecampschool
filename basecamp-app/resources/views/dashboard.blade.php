<x-student-layout>
    ...
    @php $studentDocs = $admission ? (is_string($admission->documents) ? json_decode($admission->documents, true) ?: [] : ($admission->documents ?? [])) : []; @endphp
    <div x-data="{
        searchQuery: '',
        filterCategory: '',
        ...
        unlockedProductIds: {{ json_encode($unlockedProducts) }},
        ...
        copyText(text) {
            navigator.clipboard.writeText(text).then(() => {
                this.toast = { show: true, message: 'Copied to clipboard!', type: 'success' };
        ...
        async unlockProduct(id) {
            try {
                const resp = await fetch('/products/' + id + '/unlock', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
                });
                const data = await resp.json();
                if (data.success) {
                    this.unlockedProductIds.push(id);
                    this.toast = { show: true, message: data.message || 'Resource unlocked!', type: 'success' };
                } else {
                    this.toast = { show: true, message: data.message || 'Failed to unlock resource.', type: 'error' };
                }
            } catch {
                this.toast = { show: true, message: 'Network error. Please try again.', type: 'error' };
            } finally {
    ...
    <section class="mb-5 sm:mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-6 relative z-10">
        ...
        <div class="glass-panel p-3 sm:p-6 rounded-2xl flex flex-row sm:flex-row items-center gap-3 sm:gap-6 shadow-sm w-full sm:w-auto">
            ...
            <div class="flex gap-3 sm:gap-4 shrink">
    ...
    <section class="mb-6 sm:mb-10 relative z-10">
        <div class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-amber-50/90 border border-amber-300 text-amber-900 shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <a href="{{ route('application') }}" class="w-full sm:w-auto px-5 py-2.5 bg-amber-600 text-white rounded-xl text-xs font-bold hover:bg-amber-700 transition-colors flex items-center gap-2 whitespace-nowrap justify-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 items-start relative z-10 pb-24 sm:pb-28">
        ...
        <div class="md:col-span-8 glass-panel p-4 sm:p-8 rounded-2xl sm:rounded-3xl min-h-[300px] sm:min-h-[400px] shadow-sm">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 sm:mb-8">
                ...
                <a href="{{ url('/tma') }}" class="w-full sm:w-auto text-center bg-gradient-to-r from-cyan-700 to-cyan-500 text-white px-4 sm:px-5 py-2.5 sm:py-2.5 rounded-lg flex items-center justify-center gap-2 text-xs sm:text-sm font-bold shadow-lg shadow-cyan-900/20 hover:scale-[0.98] transition-transform whitespace-nowrap min-h-[44px]">
        ...
        <!-- Resources Section -->
        <div class="md:col-span-8 glass-panel p-4 sm:p-8 rounded-2xl sm:rounded-3xl min-h-[200px] shadow-sm">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 sm:mb-6">
                <div>
                    <h2 class="text-lg sm:text-2xl font-bold tracking-tight text-slate-800 mb-0.5 sm:mb-1">Resource Library</h2>
                    <p class="text-[11px] sm:text-sm font-medium text-slate-500">Premium study materials & PDF notes</p>
                </div>
            ...
            @if($allResourceCount > 0)
                <div class="space-y-3 sm:space-y-4">
                    @foreach($allResources as $product)
                        @php
                            $fileUrls = $product->file_urls;
                            if (is_string($fileUrls)) {
                                $fileUrls = json_decode($fileUrls, true) ?: [];
                        ...
                        @endphp
                        <div x-show="(!searchQuery || '{{ $product->title }}'.toLowerCase().includes(searchQuery.toLowerCase())) && (filterCategory !== 'tma')" class="group p-3 sm:p-5 rounded-xl sm:rounded-2xl bg-[#f2f7f9] border border-transparent hover:border-slate-200 hover:bg-white transition-all duration-300 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
                            <div class="flex gap-3 sm:gap-4 w-full sm:w-auto items-start">
                                <div class="w-9 h-9 sm:w-12 sm:h-12 rounded-xl bg-white flex items-center justify-center text-[#006479] shadow-sm border border-slate-100 shrink-0">
                                    <span class="material-symbols-outlined text-base sm:text-xl">picture_as_pdf</span>
                                </div>
                                <div class="min-w-0 flex-1 sm:flex-none">
                                    <h3 class="font-bold text-slate-800 text-xs sm:text-base truncate">{{ $product->title }}</h3>
                                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">{{ ucfirst($product->category) }} | Published: {{ $product->created_at->format('d M Y') }}</p>
                                </div>
                            </div>
                            <div class="flex gap-2 w-full sm:w-auto items-center">
                                ...
                                @else
                                <a href="{{ route('downloads') }}" class="flex-1 sm:flex-initial text-center px-3 sm:px-4 py-2 sm:py-2 bg-amber-600 text-white rounded-xl text-[11px] sm:text-xs font-bold hover:bg-amber-700 transition-all shadow-sm min-h-[44px] sm:min-h-0 flex items-center justify-center gap-1.5">
                                    <span class="material-symbols-outlined text-sm">shopping_cart</span>
                                    Buy ₹{{ number_format($product->price, 0) }}
                                </a>
                                @endif
                            </div>
                        </div>
                ...
                <div x-show="searchQuery && filteredResources.length === 0" class="flex flex-col items-center justify-center py-8 px-4 border-2 border-dashed border-slate-200 rounded-2xl sm:rounded-3xl bg-white/40 text-center">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center mb-3">
                        <span class="material-symbols-outlined text-lg sm:text-xl">search_off</span>
                    </div>
                    <h3 class="font-bold text-slate-700 text-sm sm:text-base">No Matching Resources</h3>
                    <p class="text-xs text-slate-500 mt-1 max-w-[260px]">Try adjusting your search or filter to find what you're looking for.</p>
                </div>
            @else
                <div class="flex flex-col items-center justify-center py-12 sm:py-16 px-4 border-2 border-dashed border-slate-200 rounded-2xl sm:rounded-3xl bg-white/40 text-center min-h-[200px]">
                    <div class="w-12 h-12 sm:w-16 sm:h-16 bg-cyan-50 text-cyan-600 rounded-full flex items-center justify-center mb-3 sm:mb-4 shadow-inner">
                        <span class="material-symbols-outlined text-xl sm:text-2xl">folder_open</span>
                    </div>
                    <h3 class="font-bold text-slate-800 text-sm sm:text-base">No Resources Available Yet</h3>
                    <p class="text-xs text-slate-500 mt-1 max-w-[260px] sm:max-w-sm">Digital study materials will appear here once published by the administration.</p>
                </div>
            @endif
        </div>
            ...
            <div class="relative overflow-hidden p-5 sm:p-8 rounded-2xl sm:rounded-3xl bg-slate-900 text-white shadow-2xl border border-slate-700">
                ...
                <div class="relative z-10">
                    ...
                    <div class="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center justify-between mb-4 sm:mb-6 backdrop-blur-md">
                        <span id="referralCode" class="font-mono text-xs font-bold tracking-widest text-[#40cef3]">{{ Auth::user()->enrollment_number ?? 'BASE-A-24' }}</span>
                        <button @click="copyText(document.getElementById('referralCode').textContent)" class="material-symbols-outlined text-white/50 hover:text-white transition-colors text-lg">content_copy</button>
                    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-10 relative z-10">

        <!-- My Documents -->
        <div id="documents-section" class="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm">
            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                <div class="p-2 bg-cyan-50 rounded-xl border border-cyan-100">
                    <span class="material-symbols-outlined text-[#006479] text-xl">folder</span>
                </div>
                <div>
                    <h3 class="text-base sm:text-lg font-bold text-slate-800">My Documents</h3>
                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">Uploaded documents & status</p>
                </div>
            </div>
            <div class="space-y-2 sm:space-y-3">
                @php
                    $docMap = ['photo' => 'Photo', 'signature' => 'Signature', 'idProof' => 'ID Proof', 'addressProof' => 'Address Proof', 'previousMarksheet' => 'Previous Marksheet', 'categoryCertificate' => 'Category Certificate'];
                    $docIcon = ['photo' => 'image', 'signature' => 'draw', 'idProof' => 'badge', 'addressProof' => 'home', 'previousMarksheet' => 'text_snippet', 'categoryCertificate' => 'workspace_premium'];
                @endphp
                @foreach($docMap as $key => $label)
                    @php
                        $docUrl = isset($studentDocs[$key]) && $studentDocs[$key] ? asset('storage/' . $studentDocs[$key]) : null;
                        $isApproved = isset($studentDocs[$key . '_status']) && $studentDocs[$key . '_status'] === 'Approved';
                        $isRejected = isset($studentDocs[$key . '_status']) && $studentDocs[$key . '_status'] === 'Rejected';
                        $hasDoc = $docUrl !== null;
                    @endphp
                    <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            @elseif($hasDoc)
                                <span class="material-symbols-outlined text-amber-600 text-sm">hourglass_empty</span>
                            @else
                                <span class="material-symbols-outlined text-slate-300 text-sm">pending</span>
                            @endif
                            <span class="text-xs sm:text-sm font-semibold text-slate-700">{{ $label }}</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            @if($isApproved)
                                <span class="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Approved</span>
                                @if($docUrl)
                                    <a href="{{ $docUrl }}" target="_blank" class="p-1 text-slate-400 hover:text-[#006479]" title="Download"><span class="material-symbols-outlined text-sm">download</span></a>
                                @endif
                            @elseif($isRejected)
                                <span class="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">Rejected</span>
                                <a href="{{ route('application') }}" class="p-1 text-amber-500 hover:text-amber-700" title="Re-upload"><span class="material-symbols-outlined text-sm">refresh</span></a>
                            @elseif($hasDoc)
                                <span class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Pending</span>
                                @if($docUrl)
                                    <a href="{{ $docUrl }}" target="_blank" class="p-1 text-slate-400 hover:text-[#006479]" title="View"><span class="material-symbols-outlined text-sm">visibility</span></a>
                                @endif
                            @else
                                <span class="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full">Not Uploaded</span>
                            @endif
                        </div>
                    </div>
                @endforeach
            ...
            <div class="mt-3 sm:mt-4 flex gap-2">
                <a href="{{ route('application') }}" class="flex-1 py-2.5 sm:py-3 bg-[#006479]/10 text-[#006479] rounded-xl text-[10px] sm:text-xs font-bold hover:bg-[#006479]/20 transition-colors text-center block">Upload Documents</a>
                <a href="{{ route('application') }}" class="flex-1 py-2.5 sm:py-3 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] sm:text-xs font-bold hover:bg-slate-50 transition-colors text-center block">Re-upload Rejected</a>
            </div>
        </div>

        <!-- Payment Section -->
        <div id="payment-section" class="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm">
            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                ...
                <div>
                    <h3 class="text-base sm:text-lg font-bold text-slate-800">Payments</h3>
                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">Fees & transaction history</p>
                </div>
            </div>
            <div class="space-y-2 sm:space-y-3">
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <div><span class="text-xs sm:text-sm font-semibold text-slate-700 block">Admission Fee</span><span class="text-[10px] text-slate-400">₹2,500</span></div>
                    <span class="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Paid</span>
                </div>
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <div><span class="text-xs sm:text-sm font-semibold text-slate-700 block">Service Charges</span><span class="text-[10px] text-slate-400">₹500</span></div>
                    <span class="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Paid</span>
                </div>
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <div><span class="text-xs sm:text-sm font-semibold text-slate-700 block">Exam Fee</span><span class="text-[10px] text-slate-400">₹1,200</span></div>
                    <span class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Pending</span>
                </div>
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <div><span class="text-xs sm:text-sm font-semibold text-slate-700 block">TMA Fee</span><span class="text-[10px] text-slate-400">₹350/assignment</span></div>
                    <span class="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full">Per TMA</span>
                </div>
            </div>
            <div class="mt-3 sm:mt-4 grid grid-cols-2 gap-2">
                <button @click="openModal('payOnline', 'Pay Online')" class="min-h-[44px] py-2.5 bg-emerald-600 text-white rounded-xl text-xs sm:text-sm font-bold hover:bg-emerald-700 transition-colors">Pay Online</button>
                <button @click="openModal('downloadInvoice', 'Download Invoice')" class="min-h-[44px] py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs sm:text-sm font-bold hover:bg-slate-50 transition-colors">Download Invoice</button>
            </div>
        </div>

        <!-- Public Examination -->
        <div id="exam-section" class="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm">
            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                ...
                <div>
                    <h3 class="text-base sm:text-lg font-bold text-slate-800">Public Examinations</h3>
                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">Exam schedule & hall tickets</p>
                </div>
            </div>
            <div class="space-y-2 sm:space-y-3">
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs sm:text-sm font-semibold text-slate-700">Exam Fee Deadline</span>
                    <span class="text-[10px] font-bold text-purple-600">15 Jul 2026</span>
                </div>
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs sm:text-sm font-semibold text-slate-700">Hall Ticket</span>
                    <span class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Not Available</span>
                </div>
                <div class="flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs sm:text-sm font-semibold text-slate-700">Exam Schedule</span>
                    <span class="text-[10px] font-bold text-slate-500">Oct/Nov 2026</span>
                </div>
                <div class="border-t border-slate-100 pt-2 mt-2">
                    <p class="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Notifications</p>
                    <div class="p-2.5 bg-purple-50 rounded-lg border border-purple-100 flex items-start gap-2">
                        <span class="material-symbols-outlined text-purple-500 text-sm shrink-0 mt-0.5">campaign</span>
                        <div><p class="text-[10px] font-bold text-slate-700">Exam Fee Payment Open</p><p class="text-[9px] text-slate-500">Last date: 15 Jul 2026</p></div>
                    </div>
                </div>
            </div>
            <button @click="openModal('examDetails', 'Exam Details')" class="mt-3 sm:mt-4 w-full min-h-[44px] py-2.5 sm:py-3 bg-[#006479]/10 text-[#006479] rounded-xl text-xs sm:text-xs font-bold hover:bg-[#006479]/20 transition-colors">View Exam Details</button>
        </div>

        <!-- Print Section -->
        <div id="print-section" class="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm">
            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                <div class="p-2 bg-orange-50 rounded-xl border border-orange-100">
                    <span class="material-symbols-outlined text-orange-600 text-xl">print</span>
                </div>
                <div>
                    <h3 class="text-base sm:text-lg font-bold text-slate-800">Print Center</h3>
                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">Identity card, receipts & more</p>
                </div>
            </div>
            <div class="space-y-2 sm:space-y-3">
                <a href="{{ route('identity.card') }}" target="_blank" class="w-full flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100 hover:bg-white transition-all group">
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">credit_card</span><span class="text-xs sm:text-sm font-semibold text-slate-700 group-hover:text-[#006479]">Identity Card</span></div>
                    <span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">download</span>
                </a>
                <button @click="openModal('printDoc', 'Admission Acknowledgement')" class="w-full flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100 hover:bg-white transition-all group">
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">description</span><span class="text-xs sm:text-sm font-semibold text-slate-700 group-hover:text-[#006479]">Admission Acknowledgement</span></div>
                    <span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">download</span>
                </button>
                <button @click="openModal('printDoc', 'Payment Receipt')" class="w-full flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100 hover:bg-white transition-all group">
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">receipt</span><span class="text-xs sm:text-sm font-semibold text-slate-700 group-hover:text-[#006479]">Payment Receipt</span></div>
                    <span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">download</span>
                </button>
                <button @click="openModal('printDoc', 'Enrollment Details')" class="w-full flex items-center justify-between p-3 sm:p-3.5 bg-white/60 rounded-xl border border-slate-100 hover:bg-white transition-all group">
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">badge</span><span class="text-xs sm:text-sm font-semibold text-slate-700 group-hover:text-[#006479]">Enrollment Details</span></div>
                    <span class="material-symbols-outlined text-slate-400 group-hover:text-[#006479] text-lg">download</span>
                </button>
            </div>
        </div>
        ...
        <!-- Result Section -->
        <div id="results-section" class="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm">
            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                ...
                <div>
                    <h3 class="text-base sm:text-lg font-bold text-slate-800">Results</h3>
                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">Check & download result copies</p>
                </div>
            </div>
            <div class="space-y-2">
                <div class="flex items-center justify-between p-3 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs font-semibold text-slate-700">Current Exam Results</span>
                    <span class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Not Published</span>
                </div>
                <div class="flex items-center justify-between p-3 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs font-semibold text-slate-700">Previous Results Archive</span>
                    <button @click="openModal('resultArchive', 'Results Archive')" class="text-[10px] font-bold text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full hover:bg-teal-100">View Archive</button>
                </div>
            </div>
            <div class="mt-3 flex items-center justify-center py-4 px-4 border border-dashed border-slate-200 rounded-xl bg-white/40 text-center">
                <span class="material-symbols-outlined text-slate-300 text-2xl mr-2">pending</span>
                <span class="text-[11px] text-slate-500 font-medium">Results will appear here once announced by NIOS.</span>
            </div>
        </div>

        <!-- PCP Section -->
        <div id="pcp-section" class="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm">
            <div class="flex items-center gap-3 mb-4 sm:mb-5">
                <div class="p-2 bg-rose-50 rounded-xl border border-rose-100">
                    <span class="material-symbols-outlined text-rose-600 text-xl">calendar_month</span>
                </div>
                <div>
                    <h3 class="text-base sm:text-lg font-bold text-slate-800">PCP Programs</h3>
                    <p class="text-[10px] sm:text-xs text-slate-500 font-medium">Personal Contact Programs</p>
                </div>
            </div>
            <div class="space-y-2">
                <div class="flex items-center justify-between p-3 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs font-semibold text-slate-700">PCP Hall Ticket</span>
                    <span class="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Not Available</span>
                </div>
                <div class="flex items-center justify-between p-3 bg-white/60 rounded-xl border border-slate-100">
                    <span class="text-xs font-semibold text-slate-700">Study Centre</span>
                    <span class="text-[10px] text-slate-500">To be assigned</span>
                </div>
            </div>
            <div class="mt-3 p-3 bg-rose-50/60 rounded-xl border border-dashed border-rose-200 text-center">
                <span class="text-[10px] text-rose-600 font-medium">PCP schedules will appear here once published.</span>
            </div>
        </div>
    ...
    <div class="relative z-10 mb-6 sm:mb-10">
        <div class="glass-panel p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-sm">
            <h3 class="text-sm sm:text-base font-bold text-slate-800 mb-3 sm:mb-4 flex items-center gap-2">
                <span class="material-symbols-outlined text-[#006479] text-lg">bolt</span>
                Quick Actions
            </h3>
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 sm:gap-3">
                <a href="{{ url('/tma') }}" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">assignment</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">TMA</span>
                </a>
                <a href="{{ url('/downloads') }}" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">folder_open</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">Resources</span>
                </a>
                <button @click="scrollTo('print-section')" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">print</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">Print</span>
                </button>
                <button @click="openModal('payOnline', 'Pay Online')" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">payments</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">Pay</span>
                </button>
                <button @click="scrollTo('documents-section')" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">folder</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">Documents</span>
                </button>
                <button @click="scrollTo('results-section')" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">leaderboard</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">Results</span>
                </button>
                <button @click="scrollTo('pcp-section')" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">calendar_month</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">PCP</span>
                </button>
                <button @click="scrollTo('exam-section')" class="flex flex-col items-center gap-1.5 p-3 sm:p-4 bg-white/60 rounded-xl border border-slate-100 hover:bg-white hover:border-[#006479]/20 transition-all group">
                    <span class="material-symbols-outlined text-[#006479] text-xl">download</span>
                    <span class="text-[9px] sm:text-[10px] font-bold text-slate-500 group-hover:text-[#006479]">Hall Ticket</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <button @click="showComposeModal = true" class="fixed bottom-8 sm:bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-cyan-600 to-cyan-500 text-white shadow-2xl shadow-cyan-500/40 hover:scale-110 active:scale-95 transition-all duration-200 flex items-center justify-center">
    ...
    <!-- Toast Notification -->
    <div x-show="toast.show" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-4" class="fixed bottom-24 left-4 right-4 sm:left-auto sm:right-6 z-[60] max-w-sm w-full sm:w-auto" x-cloak>
    ...
    <div x-show="showComposeModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" @click.self="showComposeModal = false">
        ...
        <div @click.stop class="relative w-full max-w-lg bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 max-h-[85dvh] flex flex-col" @keydown.escape.window="showComposeModal = false">
            ...
            <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 p-4 sm:p-6 border-t border-slate-100 bg-slate-50/50">
    ...
    <div x-show="activeModal" x-cloak class="fixed inset-0 z-[170] flex items-center justify-center px-4" @keydown.escape.window="closeModal()">
        ...
        <div class="relative w-full max-w-lg bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-2xl z-10 max-h-[85dvh] overflow-y-auto">
            ...
            <!-- Pay Online -->
            <div x-show="activeModal === 'payOnline'" class="space-y-4">
                <p class="text-sm text-slate-600">Select a payment method to pay your fees online.</p>
                <div class="grid grid-cols-1 gap-3">
                    <button class="w-full p-4 bg-white rounded-xl border-2 border-slate-200 hover:border-emerald-400 text-left flex items-center gap-3 transition-all">
                        <span class="material-symbols-outlined text-emerald-600 text-2xl">credit_card</span>
                        <div><p class="text-sm font-bold text-slate-800">Pay via Card</p><p class="text-[10px] text-slate-500">Credit / Debit / Net Banking</p></div>
                    </button>
                    <button class="w-full p-4 bg-white rounded-xl border-2 border-slate-200 hover:border-emerald-400 text-left flex items-center gap-3 transition-all">
                        <span class="material-symbols-outlined text-emerald-600 text-2xl">qr_code_scanner</span>
                        <div><p class="text-sm font-bold text-slate-800">Pay via UPI</p><p class="text-[10px] text-slate-500">Google Pay / PhonePe / Paytm</p></div>
                    </button>
                </div>
                <p class="text-[10px] text-slate-400 text-center pt-2 border-t border-slate-100">Secure payments powered by Basecamp</p>
                <button @click="closeModal()" class="w-full py-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50">Cancel</button>
            </div>

            <!-- Download Invoice -->
            <div x-show="activeModal === 'downloadInvoice'" class="space-y-4">
                <p class="text-sm text-slate-600">Select an invoice to download.</p>
                <div class="space-y-2">
                    <div class="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100">
                        <span class="text-xs font-semibold text-slate-700">Admission Fee</span>
                        <button @click="closeModal(); $nextTick(() => { window.open('{{ route("downloads") }}', '_blank'); })" class="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-[10px] font-bold">Download</button>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100">
                        <span class="text-xs font-semibold text-slate-700">Service Charges</span>
                        <button @click="closeModal(); $nextTick(() => { window.open('{{ route("downloads") }}', '_blank'); })" class="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-[10px] font-bold">Download</button>
                    </div>
                </div>
                <button @click="closeModal()" class="w-full py-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50">Close</button>
            </div>

            <!-- Exam Details -->
            <div x-show="activeModal === 'examDetails'" class="space-y-4">
                <div class="bg-purple-50 rounded-xl p-4 border border-purple-100">
                    <p class="text-xs font-bold text-purple-700">{{ $examLabel }}</p>
                    <p class="text-[10px] text-purple-500 mt-1">Your public examination schedule and hall tickets will be available here once published.</p>
                </div>
                <div class="space-y-2">
                    <div class="flex justify-between p-3 bg-white rounded-xl border border-slate-100"><span class="text-xs font-semibold text-slate-700">Exam Fee Deadline</span><span class="text-[10px] text-purple-600 font-bold">15 Jul 2026</span></div>
                    <div class="flex justify-between p-3 bg-white rounded-xl border border-slate-100"><span class="text-xs font-semibold text-slate-700">Hall Ticket</span><span class="text-[10px] text-amber-600 font-bold">Not Available</span></div>
                    <div class="flex justify-between p-3 bg-white rounded-xl border border-slate-100"><span class="text-xs font-semibold text-slate-700">Exam Schedule</span><span class="text-[10px] text-slate-500">Oct/Nov 2026</span></div>
                </div>
                <button @click="closeModal()" class="w-full py-3 rounded-xl bg-purple-600 text-white text-sm font-bold hover:bg-purple-700">Close</button>
            </div>

            <!-- Print Document -->
            <div x-show="activeModal === 'printDoc'" class="space-y-4">
                <p class="text-sm text-slate-600" x-text="'Download ' + modalTitle + ' document.'"></p>
                <div class="bg-amber-50 rounded-xl p-4 border border-amber-100 flex items-center gap-3">
                    <span class="material-symbols-outlined text-amber-600 text-2xl">print</span>
                    <div><p class="text-xs font-bold text-amber-800">Print Document</p><p class="text-[9px] text-amber-600">Use browser print (Ctrl+P) or download as PDF.</p></div>
                </div>
                <div class="flex gap-3">
                    <button @click="closeModal(); setTimeout(() => window.print(), 300)" class="flex-1 py-3 bg-[#006479] text-white rounded-xl text-sm font-bold hover:bg-cyan-700">Print</button>
                    <button @click="closeModal()" class="flex-1 py-3 border border-slate-200 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-50">Cancel</button>
                </div>
            </div>

            <!-- Result Archive -->
            <div x-show="activeModal === 'resultArchive'" class="space-y-4">
                <div class="bg-teal-50 rounded-xl p-4 border border-teal-100 text-center">
                    <span class="material-symbols-outlined text-teal-500 text-3xl block mb-2">archive</span>
                    <p class="text-sm font-bold text-teal-800">Previous Results Archive</p>
                    <p class="text-[10px] text-teal-600 mt-1">Past exam results will be available here once published by NIOS.</p>
                </div>
                <button @click="closeModal()" class="w-full py-3 rounded-xl bg-teal-600 text-white text-sm font-bold hover:bg-teal-700">Close</button>
            </div>
        </div>
    </div>
